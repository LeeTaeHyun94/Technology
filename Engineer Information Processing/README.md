...
- 해싱 함수
  ...
  - 폴딩법 :  Record Key를 여러 부분으로 나누고, 나눈 부분의 각 숫자를 더하거나 XOR한 값을 주소로 사용한다.
...
- Micro Operation : 레지스터에 저장된 데이터에 의해 이루어지는 동작
  ...
  - 한 개의 Clock Pulse 동안 실행되는 기본 동작
...
- 언팩 10진법 : 1 바이트를 Zone, Digit 두 부분으로 구성
...
- 팩 10진법 : 1 바이트에 2 개의 Digit를 각각 8421 BCD 코드로 표시한다.
...
- 선점(Preemptive) 스케줄링 : SRT, RR, 다단계 큐, 다단계 피드백 큐 등
- 비선점(Non-preemptive) 스케줄링 : FCFS, SJF, HRN, 기한부, 우선순위 등
  - FCFS(First Come First Served) : 시스템에 들어와 가장 오랫동안 기다린 프로세스를 선택하여 처리
  - SJF(Shortest Job First) : 작업 시간이 짧은 프로세스를 먼저 처리
  - HRN(Highest Response Scheduling)
    - (대기 시간 + 서비스 시간) / 서비스 시간 - 클 수록 우선 순위가 높다.
...
- TCP/IP 구조 중 호스트 간 신뢰성 있는 통신을 제공하는 전송 계층의 프로토콜은 TCP, UDP
  ...
  - ARP : 논리 주소를 물리 주소로 변환시켜 주는 프로토콜 (MAC -> IP)
  - RARP : 물리 주소를 논리 주소로 변환시켜 주는 프로토콜 (IP -> MAC)
