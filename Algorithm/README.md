...
- Tarjan's algorithm
  ...
  - Cross Edge의 경우에는 간선을 통해 이미 강 연결 요소로 구성되어 있는 정점으로 간다면 상관없다.
    - 하지만 구성되어 있지 않다면 Back Edge와 마찬가지로 갈 수 있는 정점의 번호를 따로 기록해줘야 한다.
...
트리의 특징적인 단점이라고 할 수 있는 것이 균형 트리가 아니라면 트리에 노드를 연결하면서 불균형을 초래할 수 있다는 것인데, 다시 말해서 한 쪽으로만 노드가 편중(Skewed)되어 다른 쪽과 균형이 맞지 않거나 극단적일 경우 편향 트리가 구성될 수 있다는 것이다.

이를 해결하기 위해 rank 배열을 추가하여 구성되는 트리에서 해당 노드의 높이에 비례하는 값을 저장해주어 항상 높이가 더 낮은 트리를 높은 트리 밑에 넣음으로써 완벽하진 않지만 최적화가 가능하다.
...
- 크루스칼 (Kruskal)
  ...
  3. 최소 비용을 갖는 간선들을 사이클이 형성되지 않게 서로 이어주다 보면 최소 신장 트리가 완성된다.
     ...
     - 현재 선택된 간선으로 이어지는 두 정점의 root(부모 노드)가 같다면 두 정점은 같은 집합에 속해 있기 때문에 간선을 추가하게 되면 사이클이 생긴다.
