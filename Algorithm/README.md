...
* 주어진 문자열로 구성할 수 있는 모든 접미사들을 사전순으로 정렬하여 Compressed Trie로 구성한 것을 Suffix Trie라고 한다.
* 모든 Suffix에 대해 원래 문자열의 시작 문자의 인덱스로 축약 표현하여 배열로 저장한 것이 Suffix Array.

이렇게 구성한 접미사 배열을 문제 조건에서 주어지는 길이에 따라 적절하게 정렬된 상태로 갖고 있는 것이 빠른 문제 해결의 핵심이다.

1. 길이는 1, 2, 4, 8, ... (2 ^k) 꼴로 증가한다.
   - 왜냐하면 접미사의 개념 자체가 길이가 1일 때부터 시작했을 때, 해당 길이에서의 rank는 과정을 반복할 수록 이미 정해져 있기 때문에 그 다음 길이에서 이전 길이에서 구한 rank를 참조할 수 있기 때문(문자열을 반으로 잘라 생각했다고 하면 이해가 쉬운 것 같다.)
2. 길이 별로 rank를 정하는데 quick sort를 사용한다면 O(nlogn)의 시간 복잡도를 갖게 되고 더 시간복잡도를 줄이고 싶다면 길이가 1일 때는 count sort, 그 외에는 기수 정렬 (radix sort)를 이용하게 된다면 O(n)만에 접미사 배열 정렬이 가능하다. 총 시간 복잡도는 각각 O(nlog^2n), O(nlogn)
   - 기수 정렬을 쓰는 이유는 길이가 1보다 큰 상황에서는 부분 문자열을 반으로 잘라 생각했을 때, 접두사의 rank가 같을 때 접미사의 rank를 통해 해당 길이의 부분 문자열의 rank를 정해주어야 하기 때문이다.
   - 말이 기수 정렬이지 접미사, 접두사를 기준으로 count sort를 두 번하는 것이다.
...
규칙에 맞게 누적합을 이용하여 lcp 배열을 구성한다면 LCP를 쉽게 구할 수 있다.

1. 길이가 긴 쪽에서부터 rank를 비교한다. 같다면 누적합에 더해주고 다르다면 길이를 줄여준다.
2. 이 과정을 반복하여 길이 1까지 확인해주면 된다.

- rank 배열 상에서 인접한 것들끼리만 비교해서 lcp 배열을 채우면 된다.
- 왜냐하면 Suffix Trie를 생각해보면 특정 인덱스 두 개를 주어질 때, 두 정점의 LCA (Least Common Ancestor) 보다 두 정점 사이에 있는 정점들끼리 생기는 LCA의 높이가 더 높을 수 없다는 아이디어와 같다.
- 때문에 두 인덱스가 주어진다면 사이에 있는 최소 lcp 값을 구하면 RMQ (Range Minimum Query)로 생각할 수 있다는 것이다.
